...
const blankItem: AdminItem = { brand: '', name: '', category: '', description: '', price: '', sizes: [], color: '', status: 'check', images: [], published: false, sortOrder: 0 };
    ...
    <main className="admin-shell">
      ...
      <Tabs defaultValue="home" orientation="vertical" className="admin-tabs">
        ...
        <div className="admin-workspace">
          ...
          <TabsContent value="finds"><AdminTitle eyebrow="КОНТЕНТ" title="Мои находки" description="Добавляйте вещи, меняйте порядок и управляйте публикацией." action={<button className="admin-button" onClick={() => setItem({ ...blankItem, sortOrder: data.items.length + 1 })}><Plus />Добавить находку</button>} />{item && <ItemEditor value={item} busy={busy} onChange={setItem} onCancel={() => setItem(null)} onSave={saveItem} onUpload={upload} />}<div className="admin-list">{data.items.map((entry) => <article key={entry.id}><img src={entry.images[0] || 'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=300&q=75'} alt="" /><div><p>{entry.brand}</p><h3>{entry.name}</h3><span>{entry.published ? 'Опубликовано' : 'Скрыто'} · #{entry.sortOrder}</span></div><div className="admin-row-actions"><button onClick={() => setItem(entry)}>Изменить</button><button className="danger" onClick={() => action({ action: 'deleteItem', id: entry.id })}><Trash2 /></button></div></article>)}</div></TabsContent>
