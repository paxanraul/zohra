  ...
  return <>
    ...
    <section className="finds-list" id="finds-list" aria-live="polite">
      {visible.map((item) => <article className="product-card" key={item.id}>
        <button type="button" className="product-card-button" onClick={() => setSelected(item)} aria-label={`Открыть ${item.brand} — ${item.name}`}>
          <span className="product-card-image"><img src={siteAsset(item.images[0])} alt={`${item.brand} — ${item.name}`} /></span>
      ...
      {!visible.length && <div className="finds-empty"><p className="eyebrow">СКОРО</p><h2>В этом разделе пока нет находок</h2><p>Можно оставить индивидуальный запрос — я поищу нужную вещь специально для вас.</p><a className="button button-dark" href={siteRoute('/request')}>Оставить запрос <ArrowUpRight size={17} /></a></div>}
    </section>
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}>
      {selected && <DialogContent className="product-dialog" showCloseButton={false}>
        ...
        <div className="product-dialog-image"><img src={siteAsset(selected.images[0])} alt={`${selected.brand} — ${selected.name}`} /></div>
        <div className="product-dialog-copy">
          ...
          <a className="button button-dark" href={siteRoute(`/request?item=${encodeURIComponent(`${selected.brand} — ${selected.name}`)}`)}>Запросить <ArrowUpRight size={17} /></a>
        </div>
      </DialogContent>}
    </Dialog>
  </>;
